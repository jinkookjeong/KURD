 ...
 * @return void
 */
function util_checkFloatNumberFormatNotRound(floatObj, hiddenObj, intPos, floatPos, messageStr){
	...
	var floatValue = floatObj.value.split(caliberPoint)[1];
	var intValue = floatObj.value.split(caliberPoint)[0];
	...
	if(intValue !=null &&
		intValue.length > intPos){
		
		if(messageStr == null){
			util_messageAlert(invalidDigitMessage.replace("{0}", intPos).replace("{1}",floatPos) , floatObj);
		}else{
			util_messageAlert(messageStr, floatObj);
		}
	...
	//소수점자리체크
	if(floatValue != null &&
	   floatValue.length > floatPos){
		
		if(messageStr == null){
			util_messageAlert(invalidDecimalMessage.replace("{1}", floatPos), floatObj);
		}else{
			util_messageAlert(messageStr, floatObj);
		}
 ...
 * @return void
 */
/*function util_removeFloatSeparater(floatValue){
	
	while(floatValue.indexOf(countPoint)!=-1){
		
		floatValue.replace(countPoint, "");
	}
...
function util_removeFloatSeparater(floatValueStr){
	...
	if( floatValueStr.indexOf(caliberSeparater)!=-1 ){
